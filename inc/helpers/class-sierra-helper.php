...
class Sierra_Helper {
	...
	public static function get_layout( $option = '' ) {
		$layout = empty( $option ) ? get_theme_mod( 'sierra_layout', false ) : get_theme_mod( $option, false );

		if ( ! $layout ) {
			$layout = Sierra_Helper::get_blog_default();
		}
	...
	public static function posted_on( $element = 'default' ) {
		...
		switch ( $element ) {
			case 'author':
				$html = '<span class="byline">' . esc_html_e( 'by ', 'sierra' );
				...
				break;
			case 'category':
				$html = '<div class="cat-links">' . esc_html__( 'Categories: ', 'sierra' );
				...
				break;
			case 'comments':
				echo ' <span class="comments-link"><a title="' . esc_attr__( 'Comment on Post', 'sierra' ) . '" href="' . esc_url( get_the_permalink( get_the_ID() ) ) . '#comments">' . esc_html( $comments->approved ) . '</a></span>';
				break;
			case 'tags':
