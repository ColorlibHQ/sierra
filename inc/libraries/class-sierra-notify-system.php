...
/**
 * Class Sierra_Notify_System
 */
class Sierra_Notify_System extends Epsilon_Notify_System {
	/**
	...
	public static function plugin_verifier( $slug = '', $get = '', $plugin_name = '', $special = false ) {
		...
		$arr['title'] = sprintf( esc_html__( 'Install: %s', 'sierra' ), $plugin_name );
		// Translators: %s is the plugin name.
		$arr['description'] = sprintf( esc_html__( 'Please install %s in order to create the demo content.', 'sierra' ), $plugin_name );

		if ( $arr['installed'] ) {
			// Translators: %s is the plugin name
			$arr['title'] = sprintf( esc_html__( 'Activate: %s', 'sierra' ), $plugin_name );
			// Translators: %s is the plugin name
			$arr['description'] = sprintf( esc_html__( 'Please activate %s in order to create the demo content.', 'sierra' ), $plugin_name );
		}
	...
	public static function verify_upPortfolio() {
		...
		if ( file_exists( ABSPATH . 'wp-content/plugins/sierra-portfolio' ) ) {
			$arr['installed'] = true;
